...
' Idea from http://superuser.com/questions/641471/how-can-i-automatically-convert-powerpoint-to-pdf
...
If WScript.Arguments.Count <> 1 Then
    WriteLine "You need to specify input file."
    WScript.Quit
End If

inputFile = WScript.Arguments(0)
outputFile = Replace(Replace(inputFile,"\figures-pptx\","\figures\"),".pptx",".pdf")
...
If objFso.FileExists( outputFile ) Then
	Dim objTargetFile
	Dim objSourceFile
	Set objTargetFile = objFso.GetFile(outputFile)
	Set objSourceFile = objFso.GetFile(inputFile)
	If (objTargetFile.DateLastModified > objSourceFile.DateLastModified) Then
		WriteLine "PDF is older than PPTX, do nothing."
		WScript.Quit
	End If
End If
...
objPresentation.ExportAsFixedFormat outputFile, ppFixedFormatTypePDF, ppFixedFormatIntentPrint, msoFalse, ppPrintHandoutHorizontalFirst, ppPrintOutputSlides, msoFalse, objPrintOptions.Ranges(1), ppPrintAll, "Slideshow Name", False, False, False, False, False

objPresentation.Close

' Close PowerPoint if there is no other presentation opened
if(objPPT.Presentations.count < 1) Then
	objPPT.Quit
End if